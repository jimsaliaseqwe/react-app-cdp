...
export default () => {
    ...
    const isFullScreen = matches[matches.length - 1].handle !== undefined && matches[matches.length - 1].handle.fullscreen;
    ...
    const menuItems = listRoute[1].children.filter(i => typeof i['isShowMenu'] === 'undefined' || i['isShowMenu'] === true);
    ...
    useEffect(() => {
        if (window) {
            window.document.querySelector("head title").innerText = "Admin | " + window.document.querySelector("head title").innerText;
        }
    }, []);

    useEffect(() => {
        if (isAuth && location && (location.pathname === ROUTE_PATH.SMOD || location.pathname === `${ROUTE_PATH.SMOD}/`)) {
    ...
    const SADMIN = () => {
        if (isAuth) {
            if (isFullScreen) {
                return <PrimeReactProvider value={{}}><Outlet context={[toast]}/></PrimeReactProvider>;
            }
